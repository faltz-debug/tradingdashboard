...
/**
 * Readiness probe — executa um SELECT 1 contra o banco para confirmar que
 * a conexao esta viva e a tabela `users` existe (o schema CREATE TABLE
 * IF NOT EXISTS rodou). Devolve true em sucesso, false se qualquer
 * etapa falhar (banco corrompido, lock irrecuperavel, etc).
 *
 * Usado por bootstrapAdminAccount no middleware/auth.js antes de tentar
 * o ensureBootstrapAdmin, para distinguir "DB ainda inicializando" (retry)
 * de "DB quebrado" (skip + log).
 *
 * @returns {boolean}
 */
function isReady() {
